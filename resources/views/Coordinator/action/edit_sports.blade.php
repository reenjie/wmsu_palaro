   ...
   <div class="container">
        ...
        <h6 class="af mb-3" style="font-weight:bold;font-size:18px">Update Sports/Event</h6>
          ...
          <div class="card shadow-sm bg-transparent">
            <div class="card-body">
              <form action="{{route('coordinator.edit_sports')}}" method="post" enctype="multipart/form-data">
                     @csrf
             @foreach ($data as $row)
                 <input type="hidden" name="sid" value="{{$row->id}}">
     
            <div class="container">
                        <a href="{{route('coordinator.sportevents')}}" class="btn btn-dark btn-sm">Back</a>
